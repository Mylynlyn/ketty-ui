    ...
    <el-dialog :visible.sync="dialogVisible" width="60%" title="制单"  class="makeFormStyle">
      <el-form :model="addForm" ref="addForm" size="mini" :rules="addFormRules" id="print_content">
        ...
        <div class="formBorder">
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="姓名" prop="sendname" label-width="80px">
                <el-select v-model="addForm.sendname" @change="changeSendName"  style="width:100%" filterable>
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="姓名" prop="receivername" label-width="80px">
                <el-select v-model="addForm.receivername" @change="changeReceiveName"  style="width:100%" filterable>
          ...
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item label="中转站" prop="transferstation" label-width="80px">
                <el-select v-model="addForm.transferstation"  style="width:100%" filterable>
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="航班" prop="flight" label-width="80px">
                <el-select v-model="addForm.flight"  style="width:100%" @change="selectFlight" filterable>
            ...
            <el-col :span="8">
              <el-form-item label="班次" prop="flightnum" label-width="80px">
                <el-select v-model="addForm.flightnum" style="width:100%" @change="getFlightRatio" filterable>
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="注意事项" prop="attention" label-width="80px">
                <el-select v-model="addForm.attention" placeholder="请选择" style="width:100%" filterable>
          ...
          <el-table v-show='true' :data="cargoTableData" size="mini" width="100%" class="cargoTableStyle" :summary-method="getSummaries"
            ...
            <el-table-column label="毛重" prop="grossall" align="center" width="60px">
              <template slot-scope="scope">
                <el-input v-model="scope.row.grossall" v-if="scope.row.index==tabRowIndex && scope.column.index==tabColumnIndex" :size="cargoSize">
                </el-input>
            ...
            <el-table-column label="尺寸(m)" align="center">
              <el-table-column label="货物包装" prop="goodname" align="center" >
                <template slot-scope="scope">
                  <el-select v-model="scope.row.goodname" v-if="scope.row.index==tabRowIndex && scope.column.index==tabColumnIndex" :size="cargoSize" filterable>
    ...
    export default {
        ...
        methods: {
            ...
            changeDeliveryStation(val) {//改变交货运站--影响第一承运，获取货物信息表--通过航司筛选（涉及费率）
                ...
                this.$api.goods.searchGoodsList({
                    airfight:val[0]
                }).then(res=>{
                    if(res.code==200){
            ...
            changeReceiveName(val) {//收货人信息自动关联
                console.log(val)
                console.log(this.receiveNameOptions)
                for(let i=0;i<this.receiveNameOptions.length;i++){
                    if(this.receiveNameOptions[i].receivename==val){
                        this.addForm.receivertel=this.receiveNameOptions[i].receivetel
                        this.addForm.receiveradr=this.receiveNameOptions[i].receiveadr
                    }
                }
            },
            ...
            selectFlight(val){//选定航班，关联班次,获取班次费率
            ...
            changeGoodNum(val){//商品代码改变，关联运价种类和费率
                ...
                for(let i=0;i<this.goodsInfoOptions.length;i++){
                    if(this.goodsInfoOptions[i].goodnum==val){
                        ...
                        this.cargoTableData[this.tabRowIndex].goodratio=this.goodsInfoOptions[i].sendnameexp
                    }
                }
                // this.goodsInfoOptions.forEach(function (item, index) {
                //     if (item.goodnum == val) {
                //         this.cargoTableData[this.tabRowIndex].rateclass=item.goodtype
            ...
            changeGoodType(val){//运价种类改变，关联商品代码
                ...
                //         this.cargoTableData[this.tabRowIndex].itemnum=item.goodnum
                //         this.cargoTableData[this.tabRowIndex].goodratio=item.sendnameexp
                //     }
                // })
            },
            ...
            editUsedDetail(index, row) {  //修改已使用单号
                ...
                // this.cargoTableData=row.cargoTable
            },
            searchTable() { //查询
            ...
            cargoTableData:{
                handler(newarr,oldarr){ //
                    ...
                    for (let i = 0; i < this.cargoTableData.length; i++) {
                        ...
                        if(isNaN(this.cargoTableData[i].tiji) || isNaN(this.cargoTableData[i].rcp)){
                            this.cargoTableData[i].chargeweight=0
                        }else{
                            this.cargoTableData[i].chargeweight=this.cargoTableData[i].tiji*this.cargoTableData[i].rcp*1000/6 //单个计费重量
                        }
                    ...
                    for(let i=0;i<this.cargoTableData.length;i++){//记录每条数据的总计费重量，计算总航空费用，记录每条数据的总航空费用
                        ...
                        this.addForm.weightcharge= Number(this.cargoTableData[0].weightall) > Number(this.cargoTableData[0].grossall) ? Number(this.cargoTableData[0].weightall*this.cargoTableData[0].rate).toFixed(2) :Number(this.cargoTableData[0].grossall*this.cargoTableData[0].rate).toFixed(2)
